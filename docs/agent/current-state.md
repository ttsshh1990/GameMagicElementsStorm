...
- 当前 HUD 和技能释放以最多 3 个激活元素 / 技能槽为准：基础元素槽释放基础技能，高级火火火槽释放陨石；HUD 只显示冰 / 火 / 雷等级和激活数量，激活数量按当前激活槽组成拆算。
- 当前合成面板是最小背包式界面：前三格高亮表示当前激活元素，3 个合成框通过“选框后点元素”放入材料；拥有数量不再显示在 HUD。
- 当前 debug build 开局会给火 / 冰 / 雷各 3 个基础元素，并从开局解锁合成面板，方便直接测试合成；该行为由 `debug/game/start_with_three_each_basic_element=true` 和 `debug/game/unlock_synthesis_from_start=true` 控制。
